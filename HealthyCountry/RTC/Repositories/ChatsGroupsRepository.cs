...
public class ChatsGroupsRepository : IChatsGroupsRepository
{
    private readonly WebsocketContext _context;
    private readonly IMapper _mapper;

    public ChatsGroupsRepository(WebsocketContext context, IMapper mapper)
    {
        _context = context;
        _mapper = mapper;
    }
    ...
    {
        var entity = await _context.ChatsGroups.Find(x => x.Id == id).FirstOrDefaultAsync(cancellationToken);
    ...
    {
        var entities = await _context.ChatsGroups
    ...
    public async Task UpdateAsync(GroupModel model, CancellationToken cancellationToken = default)
    {
        ...
        await _context.ChatsGroups.FindOneAndReplaceAsync<GroupEntity>(x => x.Id == entity.Id, entity,
